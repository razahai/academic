...
def quickMove(board, tkn):
    ...
    ps = q_pm(board, tkn, False)
    if board == originalBoard:
        return 37
    # 21 shouldn't be here, but at one point, 21 and 19 are both in ps so i need it here
    if tkn == "x":
        if 23 in ps:
            return 23
        elif 21 in ps:
    ...
    if 64-tokencount <= HLLIM:
        ...
        return ab[-1]
    elif 64-tokencount > HLLIM:# and tokencount > 16:
...
def abmidgame(brd, tkn, lower, upper, level):
    if (time.perf_counter()-start_time) >= 0.16:
...
def snapshot(board, possible_board, tokens, possible_moves, next_plr, plr="NO_PLR", move="NO_MOVE"):
    if move != "NO_MOVE":
        ...
        possible_board = possible_board[:move] + plr[0].upper() + possible_board[move+1:]
    display(possible_board)
    print(f"{board} {tokens[0]}/{tokens[1]}")
    print(f"Possible moves for {next_plr}: {', '.join(str(move) for move in possible_moves) if possible_moves else 'No moves possible'}")
    print()
